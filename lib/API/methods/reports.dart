...
Future<Map> reportPost(Post post) async {
  return await globals.baseAPI
      .post('v2/reports/${globals.user.uid}/post', {"postID": post.postID});
}

Future<Map> reportProfile(User user) async {
  return await globals.baseAPI
      .post('v2/reports/${globals.user.uid}/profile', {"uid": user.uid});
}

Future<Map> reportComment(Post post, Comment comment) async {
  return await BaseAPI().post(
      'v2/reports/${globals.user.uid}/${post.postID}/comment',
      comment.toJson());
}
